...
# Claritas per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Targets $CLARITAS_HEALTH_URL.
...
echo "=== Claritas DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: CLARITAS_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Claritas DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Claritas DDoS Challenge: PASSED ==="
